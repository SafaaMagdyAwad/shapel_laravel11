...
@section('content')
    <div class="container my-5">
      <div class="bg-light p-5 rounded">
        <h2 class="fw-bold fs-2 mb-5 pb-2">all blogs</h2>
        ...
        <table class="table table-hover">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td> <img src="{{ asset('assets/images/blog/' . $item->image) }}"
                alt="{{$item->title}}" class="card-img"
                  width="50" height="100" /></td>
              <td>{{$item->like}} Likes</td>
              <td>{{Str::limit($item->description, 25, '...')}}</td>
              <td>{{$item->isPublished?"Yes":"NO"}}</td>
              <td><a href="{{route('blog.show',$item)}}">View</a></td>
              <td><a href="{{route('blog.edit',$item)}}">Edit</a></td>
              <td>
